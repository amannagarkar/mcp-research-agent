...
function logResult(name, status, message, data) {
...
async function runTests() {
  ...
  if (allPapers.length > 0) {
    ...
    try {
      ...
      if (paper.id === paperId) {
        ...
        console.log(`   - Authors: ${JSON.stringify(paper.authors).substring(0, 80)}...`);
        console.log(`   - Tags: ${JSON.stringify(paper.tags).substring(0, 80)}...`);
    ...
    try {
      ...
      if (paper.authors && paper.authors.length > 0) {
        ...
        if (isString || hasNameFields) {
          logResult('Authors Format', 'PASS', `Authors properly formatted as ${isString ? 'strings' : 'objects'}`, { firstAuthor, count: paper.authors.length });
        } else {
    ...
    try {
      ...
      if (paper.tags && paper.tags.length > 0) {
        ...
        if (isString || hasNameField) {
          logResult('Tags Format', 'PASS', `Tags properly formatted as ${isString ? 'strings' : 'objects'}`, { firstTag, count: paper.tags.length });
        } else {
  ...
  try {
    ...
    if (error.response && error.response.status === 404) {
      ...
      logResult('Get Non-existent Paper', 'FAIL', `Expected 404, got ${error.response ? error.response.status : 'network error'}`);
    }
  }
